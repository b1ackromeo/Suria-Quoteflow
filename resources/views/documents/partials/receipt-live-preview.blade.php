    ...
    <div class="space-y-4 p-5 text-sm text-slate-900">
        <div class="grid gap-3">
            <div class="border-l-4 border-blue-600 bg-slate-50 p-3">
                <p class="text-[11px] font-black uppercase tracking-wide text-slate-500" data-receipt-label="partyLabel">Supplier</p>
            ...
            <div class="divide-y divide-slate-200 border border-slate-200 bg-slate-50">
                <div class="flex justify-between gap-3 px-3 py-2">
                    ...
                    <span class="text-right font-black" data-preview-date>{{ optional($document->issue_date)->format('d M Y') ?? 'Received date' }}</span>
                </div>
                <div class="flex justify-between gap-3 px-3 py-2">
                    <span class="font-semibold text-slate-500" data-receipt-label="sourceLabel">Issued PO</span>
                    <span class="text-right font-black" data-preview-related>{{ $document->relatedDocument?->document_number ?? 'Select source PO' }}</span>
                </div>
                <div class="flex justify-between gap-3 px-3 py-2">
                    <span class="font-semibold text-slate-500" data-receipt-label="referenceLabel">Delivery order reference</span>
                    <span class="text-right font-black" data-preview-reference>{{ $document->external_reference ?: 'Delivery order reference' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="grid gap-2 sm:grid-cols-3">
            ...
            <div class="border border-slate-200 bg-white p-3">
                <p class="text-[10px] font-black uppercase tracking-wide text-slate-500">Matching status</p>
                <p class="mt-1 font-black" data-receipt-label="matchingStatus">Ready after goods are marked received</p>
            </div>
            <div class="border border-slate-200 bg-white p-3">
                <p class="text-[10px] font-black uppercase tracking-wide text-slate-500">Evidence</p>
                <p class="mt-1 font-black">Upload after save</p>
            </div>
        </div>

        <section class="border-t-4 border-blue-600 bg-slate-50 p-3">
            <p class="text-[11px] font-black uppercase tracking-wide text-slate-700" data-receipt-label="remarksTitle">Receiving remarks</p>
            ...
            <table class="min-w-full text-left text-xs">
                <thead class="bg-[#0a345f] text-white">
                    <tr>
                        ...
                        <th class="px-3 py-2 font-black" data-receipt-label="previewDescriptionHeader">Material / item received</th>
                        <th class="px-3 py-2 text-right font-black">PO qty</th>
                        <th class="px-3 py-2 text-right font-black" data-receipt-label="previewReceivedHeader">Received qty</th>
                        <th class="px-3 py-2 text-right font-black" data-receipt-label="previewExceptionHeader">Short / rejected</th>
        ...
        <section class="rounded-lg border border-amber-200 bg-amber-50 p-3">
            <p class="text-[11px] font-black uppercase tracking-wide text-amber-800" data-receipt-label="evidenceTitle">Evidence to attach after save</p>
